    ...
    class UserSetting {
        ...
        public bool Load() {
            ...
            try {
                ...
                if (subkey == null) status = false;
                    ...
                    _install = subkey.GetValue(REG_INSTALL_DIRECTORY, REG_VALUE_UNKNOWN) as string;
                    subkey.Close();
                }
                ...
                // パス関連
                _output = subkey.GetValue(REG_LAST_OUTPUT_ACCESS, desktop) as string;
                if (_output == null || !Directory.Exists(_output)) _output = desktop;
                _input = subkey.GetValue(REG_LAST_INPUT_ACCESS, desktop) as string;
                if (_input == null || !Directory.Exists(_input)) _input = desktop;
                _program = subkey.GetValue(REG_USER_PROGRAM, "") as string;
                if (_program == null || !File.Exists(_program)) _program = "";
        ...
        public bool Save() {
            ...
            try {
                ...
                // パス関連
                string dir = "";
                if (Directory.Exists(_output)) dir = _output;
                else if (Directory.Exists(Path.GetDirectoryName(_output))) dir = Path.GetDirectoryName(_output);
                if (dir.Length > 0) subkey.SetValue(REG_LAST_OUTPUT_ACCESS, dir);

                dir = "";
                if (Directory.Exists(_input)) dir = _input;
                else if (Directory.Exists(Path.GetDirectoryName(_input))) dir = Path.GetDirectoryName(_input);
        ...
        const string REG_INSTALL_DIRECTORY  = "InstallDirectory";
